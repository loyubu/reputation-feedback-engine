...
**The standing rule.** The system never publishes publicly and never sends anything to a
customer. It notices, scores, routes and prepares; a human does the rest. Any feature that
auto-sends or auto-posts contradicts the brief.
...
- **Two model calls, not one** — score first, draft second and only for negatives. Keeps drafting
  failures from taking the routing with them.
...
## Blocked on four decisions

Implementation does not start until the group answers these. Do not pick for them.

1. **Datastore + dashboard** — Airtable + Interface Designer · Supabase + custom dashboard ·
   Supabase + hosted page · Sheets + Looker Studio
2. **Request and reply channel** — n8n Form link (recommended) · email out and reply in ·
   Telegram bot · simulated webhook
3. **Manager alert channel** — Telegram · email · Slack · both
4. **"Ready to post" content** — the customer's raw words, or a lightly polished draft of them
...
- **Credentials do not survive an export** — they are stripped from workflow JSON. Keep a running
  list of what needs re-connecting.
